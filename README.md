...
- ダウンロードしたzipファイルを展開したフォルダの中 `sinonome_bouncer.exe` を実行してください。
...
- `frame_count`はPNG内のタイル数以下にしてください。タイルは左から右方向へ順に読み込まれます。
...
- `monitor=all`は全モニタを1つの領域として使用します。`monitor=1`や`monitor=2`のように指定すると、番号に対応するモニタ内だけで動きます。モニタ番号はWindowsのディスプレイ設定に表示される番号を指定してください。
